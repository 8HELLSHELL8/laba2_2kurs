...
//		if (sym == ' ')
//		{
//			if (!word.empty())  // Добавляем слово, если оно не пустое
//			{
//				processedWords.MPUSH(word);
//				word = "";
//			}
//		}
//		else word += sym; // Добавляем символ к слову
//	}
//	if (!word.empty()) processedWords.MPUSH(word); // Добавляем последнее слово, если оно не пустое
...
//
//		// Если слово совпадает с вариантом в словаре (регистр не учитывается)
//		if (lowerWord == toLower(dictWord))
//		{
//			// Проверка на правильность регистра
//			if (word == dictWord) return 1;  // Слово корректно написано
...
//
//		// Слово есть в словаре и написано корректно
//		if (checkResult == 1) continue;
//
//		// Слово есть в словаре, но написано некорректно
//		if (checkResult == 0)
//		{
//			incorrectCounter++;
//			incorrectWords.MPUSH(text.MGET(i));
//		}
//		// Слово не найдено в словаре, но корректно написано
