...
void process_command(const user_command& cmd, const string& name_file, const string& structure_name)
{
    ...
    if (structure_name == "set")
    {
        ...
        set<string> mnojestvo;
        mnojestvo.load_from_file(name_file, cmd.name_structure);
        if (cmd.command == "SETADD")
        {
            mnojestvo.SPUSH(cmd.value_fir);
            mnojestvo.save_to_file(name_file, cmd.name_structure);
        }
        else if (cmd.command == "SETDEL")
        {
           mnojestvo.SDEL(cmd.value_fir);
           mnojestvo.save_to_file(name_file, cmd.name_structure);
        }
        else if (cmd.command == "SETAT")
        {
           cout << mnojestvo.check(cmd.value_fir) << endl;
        }
        else if (cmd.command == "SETPRINT")
        {
            mnojestvo.print();
        }
    }
